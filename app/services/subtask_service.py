...
async def create_subtask_service(db: AsyncSession, subtask: SubtaskCreate) -> SubtaskResponse:
    new_subtask = SubTask(
        title=subtask.title,
...
async def get_task_id_by_subtask_id(db: AsyncSession, subtask_id: int) -> int:
    result = await db.execute(select(SubTask).where(SubTask.id == subtask_id))
    subtask = result.scalars().first()
    if not subtask:
        raise ValueError("Subtask not found.")
